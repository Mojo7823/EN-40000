    ...
    >
      <template #header>
        <div class="flex items-start justify-between">
          <div>
            <p class="text-xs uppercase tracking-wide text-primary-700 dark:text-primary-300 font-semibold">Risk Acceptance Criteria</p>
            <h1 class="text-3xl font-bold mt-2 text-gray-900 dark:text-white">Assessment Summary</h1>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">
              [Reference: Clause 6.3]
            </p>
          </div>
        ...
        <UCard>
          ...
          <div v-if="evidenceEntries.length === 0" class="py-8 text-center">
            ...
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-500">
              Add evidence in the Risk Management sections first.
            </p>
            <UButton
              to="/riskcrit/acceptance-criteria"
              color="primary"
              variant="soft"
              class="mt-4"
            >
              Go to Risk Acceptance Criteria
            </UButton>
          </div>
    ...
    <UCard>
      <template #header>
        <div>
          ...
          <h2 class="text-xl font-bold mt-2 text-gray-900 dark:text-white">Overall Verdict for Clause 6.3</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Select the overall assessment verdict for Risk Acceptance Criteria and Methodology.</p>
        </div>
      </template>
    ...
    <UCard>
      <template #header>
        <div>
          ...
          <h2 class="text-xl font-bold mt-2 text-gray-900 dark:text-white">Clause 6.3 Requirements</h2>
...
import { RISK_ACCEPTANCE_CRITERIA_REQUIREMENTS, RISK_EVIDENCE_STATUS_OPTIONS } from '~/services/documentWorkspace'
...
const requirementOptions = computed(() => {
  return RISK_ACCEPTANCE_CRITERIA_REQUIREMENTS.map(req => ({
    value: req.id,
...
const tableData = computed<TableRow[]>(() => {
  ...
  for (const req of RISK_ACCEPTANCE_CRITERIA_REQUIREMENTS) {
    // Add subsection header if new subsection
    if (req.subsection !== currentSubsection) {
      currentSubsection = req.subsection
      const subsectionLabels: Record<string, string> = {
        '6.3.2': '6.3.2 Input',
        '6.3.3': '6.3.3 Risk Methodology Requirements',
        '6.3-03': '6.3-03 Risk Acceptance Criteria Requirements',
        '6.3.4': '6.3.4 Output',
        '6.3.5': '6.3.5 Assessment Criteria',
      }
      rows.push({
...
const evidenceEntries = computed<RiskEvidenceEntry[]>(() => {
  ...
  if (state.riskManagement?.riskAssessmentMethodology?.evidenceEntries?.length) {
    entries.push(...state.riskManagement.riskAssessmentMethodology.evidenceEntries)
  }

  // Risk Acceptance Criteria evidence
  if (state.riskManagement?.riskAcceptanceCriteria?.evidenceEntries?.length) {
    entries.push(...state.riskManagement.riskAcceptanceCriteria.evidenceEntries)
  }
...
function generateNCId(): string {
  ...
  let counter = 1
  let newId = `NC-6.3-${String(counter).padStart(2, '0')}`
  while (existingIds.includes(newId)) {
    counter++
    newId = `NC-6.3-${String(counter).padStart(2, '0')}`
  }
  return newId
}
...
<p>This section summarizes the findings from the Risk Acceptance Criteria assessment for Clause 6.3 of EN 40000.</p>

<h4>Key Findings</h4>
<ul>
  <li><strong>Input Documentation:</strong> [Describe the status of input documentation review including product context, legal requirements, and stakeholder expectations]</li>
  <li><strong>Risk Methodology:</strong> [Summarize conformance with risk assessment and treatment methodology requirements]</li>
  <li><strong>Risk Acceptance Criteria:</strong> [Describe how risk acceptance criteria address regulatory, contractual, and product-specific factors]</li>
...
function saveState() {
  ...
  workspace.updateRiskManagementState({
    riskAcceptanceCriteriaAssessment: {
...
function hydrate(state: DocumentWorkspaceState) {
  ...
  const assessment = state.riskManagement?.riskAcceptanceCriteriaAssessment
